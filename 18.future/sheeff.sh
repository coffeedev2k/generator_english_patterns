...
# предположения - по моему мнению - p
# предположения - текущая ситуация движется к тому, что это случится - P
# намерения - только что пришла в голову идея и ее хочешь сделать - H
# намерения - запланировал что-то всерьез и надолго - h
case='h H P p'
#time='= = ='
...
for ((k = 0; k < ${COUNT_OF_FILES}; k++)); do
    ...
    for ((i = 0; i < 100; i++)); do
        ...
        # echo $fverb | cut -f $(echo $(($RANDOM % 6 + 1))) -d " " | tr -d '\r\n' >>$r
        # echo -n "     " | tr -d '\r\n' >>$r
        ...
        echo $case | cut -f $(echo $(($RANDOM % 4 + 1))) -d " " | tr -d '\r\n' >>$r
        echo -n "     " >>$r
